...
namespace ClassRoomPlanner.Seating
{
    ...
    public class SeatPlanner
    {
        ...
        public ObservableCollection<Table<Child>> GenerateRandomSeating()
        {
            int childCount = 0;
            ClearTables();
            ...
            foreach (var table in tablesInClass)
            {
                ...
                while (table.ChildrenAtTable.Count != table.NumberOfChairs)
                {
                    ...
                    table.ChildrenAtTable.Add(childrenInClass[childCount]);
                    childCount++;
                }
            }
        ...
        private void SeatGoodChildren()
        {
            ...
            foreach (var table in tablesWithSpareSeats)
            {
                ...
                while (table.ChildrenAtTable.Count != table.NumberOfChairs)
                {
                    if (goodChildren.Any())
                    {
                        Child goodChild = goodChildren.Last();
                        table.ChildrenAtTable.Add(goodChild);
                        goodChildren.RemoveAll(c => c.Id == goodChild.Id);
                    }
                    else { break; }
                }
        ...
        public ObservableCollection<Table<Child>> GenerateCantSitByTable(){
...
public ObservableCollection<Table<Child>> GenerateCantSitWithBySeat()
{
